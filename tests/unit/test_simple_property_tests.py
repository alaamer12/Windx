...
class TestSimpleProperties:
    ...
    @given(
        ...
        customer_data=simple_customer_data()
    )
    ...
    async def test_property_customer_data_consistency(self, user_data: dict, customer_data: dict):
        ...
        **Feature: entry-page-customer-rbac-fix, Property 8: Customer data consistency**
        ...
        auto_created_customer = {
            'email': user.email,
            'contact_person': user.full_name if user.full_name and user.full_name.strip() else user.username,
    ...
    @given(
        users=st.lists(simple_user_data(), min_size=2, max_size=5),
        shared_email=st.emails()
    )
    @settings(max_examples=30, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture])
    @pytest.mark.asyncio
    async def test_property_customer_creation_idempotency(self, users: list[dict], shared_email: str):
        """
    ...
    @settings(max_examples=30, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture])
    @pytest.mark.asyncio
    async def test_property_backward_compatibility_preservation(self, user_data: dict, role_changes: list[str]):
        """
        **Feature: entry-page-customer-rbac-fix, Property 7: Backward compatibility preservation**
        ...
        for role in role_changes:
            ...
            user = User(**user_data_copy, role=role, is_active=True, is_superuser=(role == Role.SUPERADMIN.value), hashed_password="test")
    ...
    @pytest.mark.asyncio
    async def test_property_casbin_policy_consistency(self, authorization_scenarios: list[tuple[str, str, str]]):
        """
        ...
        for role, resource, action in authorization_scenarios:
            ...
            def evaluate_policy(user_role: str, resource_name: str, action_name: str) -> bool:
                ...
                if user_role == Role.CUSTOMER.value:
                    if resource_name in ["configuration", "quote"] and action_name in ["read", "create"]:
                        return True
                    return False
    ...
    @pytest.mark.asyncio
    async def test_property_template_rbac_function_consistency(self, template_scenarios: list[tuple[str, str, str]]):
        """
        ...
        for role, resource, action in template_scenarios:
            ...
            def backend_policy_check(user_role: str, resource_name: str, action_name: str) -> bool:
                if user_role == Role.SUPERADMIN.value:
                    ...
                    return resource_name in ["configuration", "quote"] and action_name in ["read", "create"]
                return False
