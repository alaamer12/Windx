...
def validation_rules(draw):
    ...
    if rule_type == 'range':
        ...
        choices = draw(st.lists(st.text(min_size=1, max_size=20), min_size=2, max_size=5, unique=True))
        return {
            'choices': choices,
...
class TestEntryValidation:
    ...
    @pytest.mark.asyncio
    @given(
        schema=profile_schema_with_validation(1),
        seed=st.integers()
    )
    @settings(max_examples=100, deadline=None)
    async def test_property_schema_based_validation_enforcement(self, schema: ProfileSchema, seed: int):
        """
        **Feature: entry-page-system, Property 6: Schema-based validation enforcement**
        ...
        for field_name, error_message in exc_info.value.field_errors.items():
            ...
            assert any(keyword in error_message.lower() for keyword in [
                field_name.lower(), 'required', 'invalid', 'must', 'between', 'pattern', 'format'
            ])
    ...
    async def test_property_required_field_validation(self, required_fields: list[str]):
        ...
        for required_field in required_fields:
            if required_field not in ['name', 'type', 'material', 'opening_system', 'system_series']:
    ...
    async def test_property_cross_field_validation(self, cross_field_scenario: str):
        ...
        if cross_field_scenario == 'builtin_flyscreen_missing_dimensions':
            ...
            assert any(field in field_errors for field in [
