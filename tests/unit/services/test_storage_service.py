...
class MockStorageProvider(StorageProvider):
    ...
    async def upload_file(self, file_content: bytes, filename: str, content_type: str | None = None) -> UploadResult:
        if self.should_fail:
...
class TestFileStorageService:
    ...
    @patch('app.services.storage.service.IMAGE_PROCESSING_AVAILABLE', True)
    async def test_upload_with_image_processing_success(self, storage_service_with_image_processing, sample_image_data):
        ...
        storage_service.image_processor.process_image.assert_called_once_with(sample_image_data, "test.jpg")

    @patch('app.services.storage.service.IMAGE_PROCESSING_AVAILABLE', True)
    async def test_upload_with_image_processing_failure(self, storage_service_with_image_processing, sample_image_data):
    ...
    @patch('app.services.storage.service.IMAGE_PROCESSING_AVAILABLE', True)
    async def test_upload_with_format_change(self, storage_service_with_image_processing, sample_image_data):
