...
class TestCustomerDataConsistencyProperties:
    ...
    @given(
        users_with_same_data=st.lists(
            user_data_for_customer_creation(), 
            min_size=2, 
            max_size=3
        ).map(lambda users: [
            User(
    ...
    async def test_property_idempotent_customer_creation(
        self, 
        mock_db, 
        users_with_same_data: list[User]
    ):
        """
        ...
        assert all(result.contact_person == first_result.contact_person for result in subsequent_results)