...
class TestConditionalFieldVisibility:
    ...
    def test_equals_operator_consistency(self, field_value, expected_value):
        ...
        assert result == expected_result, f"equals({field_value}, {expected_value}) should be {expected_result}"
    ...
    @given(
        ...
        type_value=st.sampled_from(["sash", "Mullion", "Flying mullion", "glazing bead", "Interlock", "Track", "auxilary"])
    )
    ...
    def test_flyscreen_conditional_visibility(self, form_data, flyscreen_value):
        ...
        assert result == flyscreen_value, f"Flyscreen fields visibility should match builtin_flyscreen_track value: {flyscreen_value}"
    ...
    def test_flying_mullion_clearance_fields(self, form_data):
        ...
        assert result is True, "Flying mullion clearance fields should be visible when type is Flying mullion"
        
        # Test with other type
        form_data["type"] = "Frame"
        result = self.evaluator.evaluate_condition(flying_mullion_condition, form_data)
        assert result is False, "Flying mullion clearance fields should be hidden when type is not Flying mullion"
    ...
    def test_and_operator_logical_consistency(self, form_data):
        ...
        if len(field_keys) >= 2:
            ...
            assert and_result == expected_result, f"AND operator should return {expected_result} for results [{result1}, {result2}]"
    ...
    def test_or_operator_logical_consistency(self, form_data):
        ...
        if len(field_keys) >= 2:
            ...
            assert or_result == expected_result, f"OR operator should return {expected_result} for results [{result1}, {result2}]"
    
    @given(
        field_path=st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'Pc'))),
        form_data=st.dictionaries(
            keys=st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))),
            values=st.one_of(st.text(), st.integers(), st.booleans()),
            min_size=1,
    ...
    def test_field_value_retrieval_consistency(self, field_path, form_data):
        ...
        if '.' not in field_path:
            ...
            assert result == expected, f"Simple field access should return {expected} for field {field_path}"
        else:
            # Nested field access - should handle gracefully
            assert result is None or isinstance(result, (str, int, bool, type(None))), "Nested field access should return valid type or None"
    ...
    def test_real_world_conditional_scenarios(self):
        ...
        result = self.evaluator.evaluate_condition(renovation_condition, form_data)
        assert result is True, "Renovation height should be visible for Frame type with renovation enabled"
        ...
        result = self.evaluator.evaluate_condition(flyscreen_condition, form_data)
        assert result is True, "Flyscreen dimensions should be visible for Kom800 series with flyscreen enabled"
