...
class TestCSVStructurePreservation:
    ...
    @given(
        profile_data=st.fixed_dictionaries({
            ...
            'type': st.sampled_from(['Frame', 'sash', 'Mullion', 'Flying mullion', 'glazing bead', 'Interlock', 'Track', 'auxilary']),
            ...
            'flyscreen_track_height': st.one_of(st.none(), st.floats(min_value=10, max_value=100)),
            ...
            'glazing_undercut_height': st.one_of(st.none(), st.floats(min_value=1, max_value=50)),
            'pic': st.one_of(st.none(), st.text(min_size=1, max_size=100)),
            'sash_overlap': st.one_of(st.none(), st.floats(min_value=1, max_value=50)),
            'flying_mullion_horizontal_clearance': st.one_of(st.none(), st.floats(min_value=1, max_value=100)),
            'flying_mullion_vertical_clearance': st.one_of(st.none(), st.floats(min_value=1, max_value=100)),
            'steel_material_thickness': st.one_of(st.none(), st.floats(min_value=0.5, max_value=10.0)),
            'weight_per_meter': st.one_of(st.none(), st.floats(min_value=0.1, max_value=50.0)),
            'reinforcement_steel': st.one_of(st.none(), st.lists(st.text(min_size=1, max_size=20), min_size=0, max_size=5)),
            'colours': st.one_of(st.none(), st.lists(st.text(min_size=1, max_size=20), min_size=0, max_size=5)),
            'price_per_meter': st.one_of(st.none(), st.decimals(min_value=Decimal('0.01'), max_value=Decimal('1000.00'))),
            'price_per_beam': st.one_of(st.none(), st.decimals(min_value=Decimal('0.01'), max_value=Decimal('10000.00'))),
            'upvc_profile_discount': st.one_of(st.none(), st.floats(min_value=0.0, max_value=100.0))
        })
    )
    def test_preview_table_has_exact_csv_structure(self, profile_data):
        ...
        # Test header count
        assert len(preview_table.headers) == 29, f"Preview table should have exactly 29 headers, got {len(preview_table.headers)}"
        
        # Test header content matches expected CSV headers
        assert preview_table.headers == self.expected_headers, "Preview table headers should match exact CSV structure"
    ...
    @given(
        form_data=st.dictionaries(
            keys=st.sampled_from([
                'name', 'type', 'company', 'material', 'opening_system', 'system_series',
                'code', 'length_of_beam', 'renovation', 'width', 'builtin_flyscreen_track',
                'total_width', 'flyscreen_track_height', 'front_height', 'rear_height',
                'glazing_height', 'renovation_height', 'glazing_undercut_height', 'pic',
                'sash_overlap', 'flying_mullion_horizontal_clearance', 'flying_mullion_vertical_clearance',
                'steel_material_thickness', 'weight_per_meter', 'reinforcement_steel', 'colours',
                'price_per_meter', 'price_per_beam', 'upvc_profile_discount'
            ]),
            values=st.one_of(
                st.none(),
    ...
    def test_header_to_field_mapping_consistency(self, form_data):
        ...
        for header in preview_table.headers:
            assert header in self.header_field_mapping, f"Header '{header}' should have field mapping"
            ...
            if field_name in form_data:
                ...
                if form_value is not None and form_value != '' and not (isinstance(form_value, list) and len(form_value) == 0):
                    # Value should be present in preview (not N/A)
                    assert row_value != 'N/A', f"Non-empty field {field_name} should not show as N/A in preview"
                else:
                    # Empty/null values should show as N/A
                    assert row_value == 'N/A', f"Empty field {field_name} should show as N/A in preview"
            else:
                # Missing fields should show as N/A
                assert row_value == 'N/A', f"Missing field {field_name} should show as N/A in preview"
    
    @given(
        null_fields=st.lists(
            st.sampled_from([
                'company', 'code', 'length_of_beam', 'renovation', 'width', 'builtin_flyscreen_track',
                'total_width', 'flyscreen_track_height', 'front_height', 'rear_height',
                'glazing_height', 'renovation_height', 'glazing_undercut_height', 'pic',
                'sash_overlap', 'flying_mullion_horizontal_clearance', 'flying_mullion_vertical_clearance',
                'steel_material_thickness', 'weight_per_meter', 'reinforcement_steel', 'colours',
    ...
    def test_null_value_handling_in_csv_structure(self, null_fields):
        ...
        assert len(preview_table.headers) == 29, "Table should maintain 29 columns even with null values"
        ...
        for field in null_fields:
            ...
            if header:
                assert row[header] == 'N/A', f"Null field {field} should show as N/A in column {header}"
    ...
    def test_boolean_field_formatting_in_csv(self, boolean_fields):
        ...
        for field_name, field_value in boolean_fields.items():
            ...
            if header:
                ...
                assert row[header] == expected_value, f"Boolean field {field_name} should format as '{expected_value}'"
        ...
        for field_name, field_value in array_fields.items():
            ...
            if header:
                if len(field_value) > 0:
                    ...
                    assert row[header] == expected_value, f"Array field {field_name} should format as comma-separated list"
                else:
                    assert row[header] == 'N/A', f"Empty array field {field_name} should show as N/A"
    ...
    def test_price_field_formatting_in_csv(self, numeric_fields):
        ...
        for field_name, field_value in numeric_fields.items():
            ...
            if header:
                ...
                assert row[header] == expected_value, f"Price field {field_name} should format with 2 decimal places"
    ...
    def test_csv_structure_completeness(self):
        ...
        for i, expected_header in enumerate(self.expected_headers):
            assert preview_table.headers[i] == expected_header, f"Header at position {i} should be '{expected_header}'"
