...
def attribute_node_strategy(draw):
    ...
    # Generate LTREE path
    section = draw(st.sampled_from(["basic_information", "dimensions", "technical_specs", "pricing"]))
    ...
    return AttributeNode(
        ...
        validation_rules=draw(st.one_of(
            st.none(),
            st.dictionaries(
        ...
        display_condition=draw(st.one_of(
            st.none(),
            st.dictionaries(
...
class TestSchemaGeneration:
        ...
        **Validates: Requirements 1.1, 1.2, 5.1, 5.2**
        ...
        for section in schema.sections:
            if len(section.fields) > 1:
                ...
                assert len(field_names_in_section) == len(set(field_names_in_section))  # No duplicates
    ...
    def test_section_organization_consistency(self, section_names):
        ...
        for section in schema.sections:
            section_nodes = [n for n in nodes if entry_service.get_section_name(n.ltree_path) == section.title]
