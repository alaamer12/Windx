...
async def create_factory_manufacturing_data(
    ...
    async def create_node_recursive(
        ...
        if current_depth == 0:
            ...
            selected_categories = random.sample(
                CATEGORIES, min(root_leaves, len(CATEGORIES))
            )
        ...
        # Depth 2+: Create options under attributes
            ...
            parent_node = next(
                (n for n in created_nodes if n.id == parent_id), None
            )
            if not parent_node:
                return
    ...
    return {
        ...
        "deepest_path": max(
            (node.ltree_path for node in created_nodes), key=lambda x: x.count(".")
        )
        if created_nodes
        else None,
    }
...
async def delete_factory_manufacturing_data(
    ...
    for mfg_type in mfg_types:
        ...
        deleted_types.append(
            {"id": mfg_type.id, "name": mfg_type.name, "nodes": len(nodes)}
        )
...
async def create_factory_customers(
    ...
    company_suffixes = ["Construction", "Builders", "Contractors", "Homes", "Properties", "Development"]
    first_names = ["John", "Jane", "Michael", "Sarah", "David", "Emily", "Robert", "Lisa", "James", "Maria"]
    last_names = ["Smith", "Johnson", "Williams", "Brown", "Jones", "Garcia", "Miller", "Davis", "Rodriguez", "Martinez"]
    streets = ["Main St", "Oak Ave", "Maple Dr", "Pine Rd", "Cedar Ln", "Elm St", "Park Ave", "Lake Dr"]
    cities = ["Springfield", "Riverside", "Fairview", "Georgetown", "Madison", "Franklin", "Clinton", "Salem"]
    ...
    customers_by_type = {}
    for customer in created_customers:
        customers_by_type[customer.customer_type] = customers_by_type.get(customer.customer_type, 0) + 1
...
async def delete_factory_customers(
    db: AsyncSession, email_pattern: str = "%@%"
) -> dict[str, Any]:
    ...
    result = await db.execute(
        select(Customer).where(Customer.email.like(email_pattern))
    )
