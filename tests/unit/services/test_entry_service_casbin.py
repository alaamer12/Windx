...
from app.services.entry import EntryService, ConfigurationCreator, ConfigurationViewer, AdminAccess
...
class TestEntryServiceCasbin:
    ...
    @pytest.mark.asyncio
    async def test_customer_creation_integrity_error_race_condition(self, mock_db, sample_user, sample_customer):
        ...
        mock_result = AsyncMock()
        mock_result.scalar_one_or_none.side_effect = [None, sample_customer]  # First None, then found
        mock_db.execute.return_value = mock_result
    ...
    @pytest.mark.asyncio
    async def test_save_profile_configuration_casbin_decorator_authorization(self, mock_db, sample_user, sample_customer, sample_manufacturing_type, sample_profile_data):
    ...
    @pytest.mark.asyncio
    async def test_save_profile_configuration_uses_customer_id(self, mock_db, sample_user, sample_customer, sample_manufacturing_type, sample_profile_data):
    ...
    @pytest.mark.asyncio
    async def test_manufacturing_type_not_found_error(self, mock_db, sample_user, sample_profile_data):
    ...
    async def test_customer_data_mapping_from_user_fields(self, mock_db):
        ...
        assert added_customer.contact_person == user.full_name  # Should use full_name when available
