...
from app.schemas.entry import ProfileEntryData, PreviewTable
...
def profile_data_with_nulls(draw):
    ...
    optional_fields = [
        "company", "code", "length_of_beam", "renovation", "width",
        "builtin_flyscreen_track", "total_width", "flyscreen_track_height",
        "front_height", "rear_height", "glazing_height", "renovation_height",
        "glazing_undercut_height", "pic", "sash_overlap",
        "flying_mullion_horizontal_clearance", "flying_mullion_vertical_clearance",
        "steel_material_thickness", "weight_per_meter", "reinforcement_steel",
        "colours", "price_per_meter", "price_per_beam"
    ]
    ...
    for field in optional_fields:
        ...
        if value_type == "null":
            ...
            elif field in ["length_of_beam", "width", "total_width", "flyscreen_track_height",
                          "front_height", "rear_height", "glazing_height", "renovation_height",
                          "glazing_undercut_height", "sash_overlap", "flying_mullion_horizontal_clearance",
                          "flying_mullion_vertical_clearance", "steel_material_thickness", "weight_per_meter"]:
...
def configuration_with_null_selections(draw):
    ...
    for i, field_name in enumerate(field_names):
        ...
        null_type = draw(st.sampled_from(["all_null", "string_value", "numeric_value", "boolean_value", "json_value"]))
        ...
        if null_type == "all_null":
            ...
            selection.string_value = None
            selection.numeric_value = draw(st.one_of(st.none(), st.floats(min_value=0, max_value=1000)))
            selection.boolean_value = None
            selection.json_value = None
            ...
            selection.numeric_value = None
            selection.boolean_value = None
            selection.json_value = draw(st.one_of(st.none(), st.lists(st.text(max_size=10), max_size=3)))
    ...
    async def test_property_graceful_null_value_handling_in_validation(
        ...
        **Feature: entry-page-system, Property 5: Graceful null value handling**
        ...
        try:
            ...
            assert "null" not in str(e).lower() or "required" in str(e).lower(), f"Unexpected null error: {e}"
        ...
        try:
            ...
            for header in preview_table.headers:
                ...
                if not form_data.get(header.lower().replace(" ", "_").replace("\n", "_")):
                    assert value in ["N/A", "no", ""], f"Unexpected null representation for {header}: {value}"
    ...
    def test_property_format_preview_value_null_handling(self):
        ...
        **Feature: entry-page-system, Property 5: Graceful null value handling**
        ...
        for input_value, expected_output in test_cases:
            ...
            assert result == expected_output, f"Unexpected format for {input_value}: got {result}, expected {expected_output}"
    ...
    @given(
        field_values=st.dictionaries(
            st.sampled_from([
                ...
                "width", "renovation", "builtin_flyscreen_track", "colours"
            ]),
            st.one_of(st.none(), st.text(max_size=20), st.booleans(), st.lists(st.text(max_size=5), max_size=3)),
            min_size=3,
            max_size=8
        )
    )
    def test_property_field_visibility_with_nulls(self, field_values: dict):
        ...
        **Feature: entry-page-system, Property 5: Graceful null value handling**
        ...
        for condition in test_conditions:
            try:
                result = entry_service.condition_evaluator.evaluate_condition(condition, field_values)
                
                # Should return a boolean result
                assert isinstance(result, bool), f"Non-boolean result for condition {condition}: {result}"
