...
class TestPageTypeDatabaseIntegration:
    ...
    async def test_page_type_field_exists(self, db_session: AsyncSession):
        ...
        result = await db_session.execute(
            text("SELECT column_name FROM information_schema.columns WHERE table_name = 'attribute_nodes' AND column_name = 'page_type'")
        )
        column = result.scalar_one_or_none()
        assert column == "page_type", "page_type column should exist in attribute_nodes table"

    async def test_page_type_default_value(self, db_session: AsyncSession, test_manufacturing_type: ManufacturingType):
    ...
    async def test_ltree_path_with_page_types(
        ...
        result = await db_session.execute(
            text("SELECT * FROM attribute_nodes WHERE ltree_path ~ 'profile.*' AND page_type = 'profile'")
        )
        ...
        result = await db_session.execute(
            text("SELECT * FROM attribute_nodes WHERE ltree_path ~ 'accessories.*' AND page_type = 'accessories'")
        )
