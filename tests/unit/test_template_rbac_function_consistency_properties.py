...
def resource_action_for_templates(draw):
    ...
    resource = draw(st.sampled_from([
        "configuration", "quote", "order", "customer", 
        "manufacturing_type", "template"
    ]))
...
class TestTemplateRBACFunctionConsistencyProperties:
        ...
        **Feature: entry-page-customer-rbac-fix, Property 11: Template RBAC function consistency**
        ...
        def mock_check_permission(user_param, resource_param, action_param):
            ...
            if user_param.role == Role.SUPERADMIN.value:
                return True
            elif user_param.role in [Role.SALESMAN.value, Role.DATA_ENTRY.value, Role.PARTNER.value]:
                return True  # Full privileges initially
            elif user_param.role == Role.CUSTOMER.value:
                return resource_param in ["configuration", "quote"] and action_param in ["read", "create"]
            return False
    ...
    @given(
        ...
        roles_to_check=st.lists(
            st.sampled_from([Role.SUPERADMIN, Role.SALESMAN, Role.DATA_ENTRY, Role.PARTNER, Role.CUSTOMER]),
            min_size=1,
            max_size=3
        )
    )
    ...
    async def test_property_template_rbac_has_role_function_consistency(
        ...
        for role_to_check in roles_to_check:
            ...
            expected_result = (
                user.role == role_to_check.value or 
    ...
    @given(
        ...
        resource_types=st.lists(
            st.sampled_from(["configuration", "quote", "order", "customer"]),
            min_size=1,
            max_size=3
        )
    )
    ...
    async def test_property_template_rbac_owns_function_consistency(
        ...
        mock_rbac_service.check_resource_ownership = AsyncMock(side_effect=mock_check_resource_ownership)
        
        # Act & Assert for each resource type
        for resource_type in resource_types:
            backend_result = await mock_rbac_service.check_resource_ownership(user, resource_type, customer.id)
            ...
            # Verify backend was called correctly
            mock_rbac_service.check_resource_ownership.assert_called_with(user, resource_type, customer.id)

    @given(
        user=user_with_role_for_templates(),
        privilege_data=st.tuples(
            st.lists(st.sampled_from([Role.SALESMAN, Role.PARTNER, Role.CUSTOMER]), min_size=1, max_size=2),
    ...
    async def test_property_template_rbac_has_privilege_function_consistency(
        ...
        def mock_check_privilege(user_param, privilege_param):
            ...
            if user_param.role == Role.CUSTOMER.value:
                has_permission = (
                    privilege_param.permission.resource in ["configuration", "quote"] and
                    privilege_param.permission.action in ["read", "create"]
                )
    ...
    @given(
        ...
        template_functions=st.lists(
            st.sampled_from(["can", "has_role", "owns", "has_privilege"]),
            min_size=3,
            max_size=5
        )
    )
