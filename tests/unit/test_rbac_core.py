    ...
    async def test_check_ownership_requirement(self, user):
        ...
        with patch('app.core.rbac.rbac_service') as mock_rbac:
            ...
            result = await _check_ownership_requirement(
                user, ownership, mock_func, args, kwargs
            )
            assert result is True
            
            mock_rbac.check_resource_ownership.assert_called_once_with(
    ...
    async def test_check_privilege_requirement(self, user):
        ...
        with patch('app.core.rbac.rbac_service') as mock_rbac:
            ...
            result = await _check_privilege_requirement(
                user, privilege, mock_func, args, kwargs
            )
            assert result is True
...
class TestRequireDecorator:
    ...
    async def test_require_decorator_with_permission(self, user):
        ...
        with patch('app.core.rbac._check_permission_requirement', new_callable=AsyncMock, return_value=True):
            result = await test_function(user)
            assert result == "success"
    ...
    async def test_require_decorator_with_ownership(self, user):
        ...
        with patch('app.core.rbac._check_ownership_requirement', new_callable=AsyncMock, return_value=True):
