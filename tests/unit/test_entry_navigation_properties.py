...
from app.api.v1.endpoints.entry import profile_page, accessories_page, glazing_page
...
def mock_request_with_params(draw):
    ...
    request.url.path = draw(st.sampled_from(["/api/v1/entry/profile", "/api/v1/entry/accessories", "/api/v1/entry/glazing"]))
    request.headers = {}
...
class TestEntryNavigationProperties:
    ...
    async def test_property_navigation_state_preservation_accessories(
        self, request, user
    ):
        """
        **Feature: entry-page-system, Property 8: Navigation state preservation**
    ...
    async def test_property_navigation_state_preservation_glazing(
        self, request, user
    ):
        """
        **Feature: entry-page-system, Property 8: Navigation state preservation**
    ...
    @given(
        page_transitions=st.lists(
            st.sampled_from(["profile", "accessories", "glazing"]),
            min_size=2,
            max_size=5
        ),
        user=mock_user()
    ...
    def test_property_query_parameter_preservation(
        self, navigation_sequence: list[dict], user
    ):
        """
        **Feature: entry-page-system, Property 8: Navigation state preservation**
