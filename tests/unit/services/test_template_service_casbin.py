...
class TestTemplateServiceCasbin:
    ...
    async def test_apply_template_uses_proper_customer_relationship(
        self, mock_db, sample_user, sample_customer, sample_template, sample_template_selections
    ):
        ...
        template_service.template_selection_repo.get_by_template = AsyncMock(
            return_value=sample_template_selections
        )
        ...
        result = await template_service.apply_template_to_configuration(
            template_id=1, user=sample_user, config_name="Custom Configuration"
        )
    ...
    async def test_apply_template_casbin_decorator_authorization(
        self, mock_db, sample_user, sample_template
    ):
        ...
        with patch("app.core.rbac.require") as mock_require:
            ...
            # Execute
            await template_service.apply_template_to_configuration(template_id=1, user=sample_user)
    ...
    async def test_template_usage_tracking_with_proper_customer_associations(
        self, mock_db, sample_user, sample_customer, sample_template, sample_configuration
    ):
        ...
        await template_service.track_template_usage(
            template_id=1, config_id=sample_configuration.id, customer_id=sample_customer.id
        )
    ...
    async def test_get_template_multiple_decorators_or_logic(
        self, mock_db, sample_user, sample_data_entry, sample_admin, sample_template
    ):
    ...
    async def test_create_template_casbin_authorization(
        self, mock_db, sample_data_entry, sample_salesman, sample_manufacturing_type
    ):
        ...
        with patch.object(ManufacturingTypeRepository, "get") as mock_mfg_get:
            ...
            with patch("app.core.rbac.require") as mock_require:
                ...
                template_data = ConfigurationTemplateCreate(
                    name="New Template", manufacturing_type_id=1, template_type="standard"
                )
    ...
    async def test_template_application_with_selections_and_customer_tracking(
        ...
        # Execute
        result = await template_service.apply_template_to_configuration(
            template_id=1, user=sample_user, config_name="Applied Template Configuration"
        )
    ...
    async def test_manufacturing_type_not_found_in_create_template(
        self, mock_db, sample_data_entry
    ):
