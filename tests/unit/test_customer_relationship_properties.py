...
def manufacturing_type_data(draw):
    ...
    return {
        ...
        'base_price': draw(st.decimals(min_value=Decimal('0.01'), max_value=Decimal('10000.00'), places=2)),
        'base_weight': draw(st.decimals(min_value=Decimal('0.01'), max_value=Decimal('1000.00'), places=2)),
        'is_active': True
    }
...
class TestCustomerRelationshipProperties:
    ...
    @pytest.mark.asyncio
    @given(user_data=user_data(), manufacturing_type_data=manufacturing_type_data(), profile_data=profile_entry_data())
    @settings(max_examples=100, deadline=None)
    async def test_property_1_customer_relationship_integrity(self, user_data, manufacturing_type_data, profile_data):
