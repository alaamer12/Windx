...
class TestRBACProperties:
    ...
    async def test_multiple_decorator_or_logic_property(self, user_role):
        ...
        user_has_required_role = (
            user_role == Role.SUPERADMIN.value or 
            user_role == Role.SALESMAN.value or
    ...
    @pytest.mark.asyncio
    @given(
        role_value=st.sampled_from([role.value for role in Role])
    )
    ...
    @given(
        user_role=st.sampled_from([role.value for role in Role]),
        resource=st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))),
        action=st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd')))
    )
    ...
    @given(
        roles=st.lists(
            st.sampled_from([role for role in Role]),
            min_size=1,
            max_size=3,
            unique=True
        )
    ...
    async def test_privilege_composition_property(self, roles):
        ...
        for test_role in Role:
            ...
            with patch('app.core.rbac.rbac_service.check_permission', new_callable=AsyncMock) as mock_check:
                ...
                # Determine expected result
                user_has_role = (
                    test_role.value == Role.SUPERADMIN.value or
                    any(test_role == role for role in roles)
                )
    ...
    @pytest.mark.asyncio
    @given(
        resource_type=st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=('Lu', 'Ll'))),
