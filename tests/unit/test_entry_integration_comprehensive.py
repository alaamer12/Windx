...
def complete_profile_workflow_data(draw):
    ...
    return {
        ...
        "profile_data": ProfileEntryData(
            ...
            price_per_meter=draw(st.one_of(st.none(), st.decimals(min_value=10, max_value=500, places=2)))
        )
    }
...
def mock_complete_database_state(draw):
    ...
    attribute_nodes = []
    field_names = ["name", "type", "material", "opening_system", "system_series", "width", "renovation"]
    ...
    for i, field_name in enumerate(field_names):
        ...
        node.depth = 1
        node.sort_order = i
        node.required = field_name in ["name", "type", "material", "opening_system", "system_series"]
...
class TestEntryIntegrationComprehensive:
    ...
    @pytest.mark.asyncio
    @given(
        workflow_data=complete_profile_workflow_data(),
        db_state=mock_complete_database_state()
    )
    async def test_complete_profile_entry_workflow(
        self, workflow_data: dict, db_state: dict
    ):
        """
        **Feature: entry-page-system, Integration Testing**
    ...
    @given(
        error_scenarios=st.lists(
            st.fixed_dictionaries({
                ...
                "error_type": st.sampled_from(["not_found", "validation", "database", "authorization"]),
    ...
    async def test_error_scenarios_and_recovery(
        self, error_scenarios: list[dict], db_state: dict
    ):
        """
        **Feature: entry-page-system, Integration Testing**
        ...
        for scenario in error_scenarios:
            ...
            if error_type == "not_found":
                mock_db.execute.side_effect = lambda stmt: AsyncMock(scalar_one_or_none=AsyncMock(return_value=None))
    ...
    def test_performance_with_large_datasets(
        self, performance_data: dict, db_state: dict
    ):
        """
        **Feature: entry-page-system, Integration Testing**
    ...
    def test_cross_browser_compatibility_simulation(
        self, cross_browser_scenarios: list[dict]
    ):
        """
        **Feature: entry-page-system, Integration Testing**
    ...
    async def test_concurrent_user_simulation(
        self, concurrent_users: int, db_state: dict
    ):
        """
        **Feature: entry-page-system, Integration Testing**
        ...
        tasks = [
            load_schema_for_user(service, i) 
            for i, service in enumerate(entry_services)
        ]
