...
def valid_profile_data(draw):
    ...
    return ProfileEntryData(
        ...
        price_per_meter=draw(st.one_of(st.none(), st.decimals(min_value=1, max_value=1000, places=2))),
...
class TestEntryDataPersistence:
    ...
    async def test_property_configuration_data_persistence(
        self, profile_data: ProfileEntryData, user_id: int, customer_id: int, manufacturing_type_id: int
    ):
        """
        **Feature: entry-page-system, Property 7: Configuration data persistence**
        ...
        entry_service.rbac_service.get_or_create_customer_for_user = AsyncMock(return_value=mock_customer)
        ...
        try:
            ...
            selection_calls = [call[0][0] for call in add_calls[1:]]  # Remaining calls should be selections
            ...
            for selection in selection_calls:
                ...
                value_fields = [
                    selection.string_value, selection.numeric_value, 
                    selection.boolean_value, selection.json_value
                ]
    ...
    async def test_property_save_load_round_trip_consistency(
        ...
        **Feature: entry-page-system, Property 7: Configuration data persistence**
        ...
        try:
            ...
            entry_service.save_profile_configuration.assert_called_once_with(profile_data, mock_user)
            entry_service.load_profile_configuration.assert_called_once_with(saved_config.id, mock_user)
