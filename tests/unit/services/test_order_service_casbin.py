...
from app.services.order import OrderService, OrderManagement, OrderReader, AdminOrderAccess
...
class TestOrderServiceCasbin:
    ...
    @pytest.mark.asyncio
    async def test_create_order_maintains_customer_relationship_from_quote(self, mock_db, sample_user, sample_quote):
    ...
    @pytest.mark.asyncio
    async def test_create_order_casbin_decorator_authorization(self, mock_db, sample_user, sample_quote):
    ...
    @pytest.mark.asyncio
    async def test_get_order_multiple_decorators_or_logic(self, mock_db, sample_user, sample_admin, sample_order):
    ...
    @pytest.mark.asyncio
    async def test_rbac_query_filter_order_filtering_by_customer_ownership(self, mock_db, sample_user):
        ...
        with patch.object(RBACQueryFilter, 'filter_orders') as mock_filter:
            ...
            filtered_query = original_query.join(Quote).where(Quote.customer_id.in_(accessible_customers))
    ...
    @pytest.mark.asyncio
    async def test_order_customer_relationship_consistency_through_quotes(self, mock_db, sample_user, sample_quote):
    ...
    @pytest.mark.asyncio
    async def test_order_already_exists_validation(self, mock_db, sample_user, sample_quote, sample_order):
        """Test validation that order cannot be created if one already exists for the quote."""
        # Setup
        order_service = OrderService(mock_db)
        order_service.quote_repo.get = AsyncMock(return_value=sample_quote)
        order_service.order_repo.get_by_quote = AsyncMock(return_value=sample_order)  # Existing order
    ...
    @pytest.mark.asyncio
    async def test_order_authorization_through_quote_customer_relationships(self, mock_db, sample_user, sample_order, sample_quote):
