...
def profile_data_with_errors(draw):
    ...
    optional_fields = {
        ...
        "width": draw(st.one_of(st.none(), st.floats(min_value=-100, max_value=1000))),  # May be negative
        "height": draw(st.one_of(st.none(), st.floats(min_value=-100, max_value=1000))),
        "price_per_meter": draw(st.one_of(st.none(), st.decimals(min_value=-1000, max_value=10000, places=2))),
...
class TestEntryErrorRecoveryProperties:
            ...
            for field_name, error_message in e.field_errors.items():
                ...
                for field in fields:
                    if field.name == field_name:
                        ...
                        assert (field.label.lower() in error_message.lower() or 
                               field.name.lower() in error_message.lower())
                        field_found = True
                        break
                
                if not field_found:
                    # Cross-field validation errors are also acceptable
                    assert field_name in ["total_width", "flyscreen_track_height", 
    ...
    async def test_property_error_recovery_not_found_errors(
        self, manufacturing_type_id: int, user
    ):
        """
        **Feature: entry-page-system, Property 10: Error recovery and user experience**
        ...
        for field_name, field_value in field_values.items():
            if field_value is not None:
                ...
                if error_message:
                    ...
                    helpful_words = ["must", "should", "required", "between", "at least", "at most", "format", "valid"]
    ...
    @pytest.mark.asyncio
    @given(
        original_data=profile_data_with_errors(),
        user=mock_user()
    )
    ...
    @given(
        error_scenarios=st.lists(
            st.fixed_dictionaries({
                "error_type": st.sampled_from(["validation", "not_found", "authorization", "database"]),
        ...
        for scenario in error_scenarios:
            ...
            if error_type == "validation":
                ...
                if error_msg:
                    ...
                    assert (error_msg.startswith(field_label) or 
                           field_label.lower() in error_msg.lower())
                    
                    # Should end with period or be complete sentence
                    assert error_msg.endswith('.') or error_msg.endswith('!') or len(error_msg.split()) > 3
