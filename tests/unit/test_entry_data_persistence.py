...
def attribute_node_data(draw, manufacturing_type_id):
    ...
    field_names = [
        ...
        'steel_material_thickness', 'weight_per_meter', 'reinforcement_steel',
        'colours', 'price_per_meter', 'price_per_beam', 'upvc_profile_discount'
    ]
...
def profile_entry_data(draw, manufacturing_type_id):
    ...
    return ProfileEntryData(
        ...
        flyscreen_track_height=draw(st.one_of(st.none(), st.floats(min_value=5.0, max_value=100.0))),
        ...
        glazing_undercut_height=draw(st.one_of(st.none(), st.floats(min_value=10.0, max_value=200.0))),
        ...
        flying_mullion_horizontal_clearance=draw(st.one_of(st.none(), st.floats(min_value=0.1, max_value=5.0))),
        flying_mullion_vertical_clearance=draw(st.one_of(st.none(), st.floats(min_value=0.1, max_value=5.0))),
        steel_material_thickness=draw(st.one_of(st.none(), st.floats(min_value=0.5, max_value=5.0))),
        weight_per_meter=draw(st.one_of(st.none(), st.floats(min_value=0.1, max_value=20.0))),
        reinforcement_steel=draw(st.one_of(st.none(), st.lists(st.text(min_size=1, max_size=50), min_size=1, max_size=3))),
        colours=draw(st.one_of(st.none(), st.lists(st.text(min_size=1, max_size=20), min_size=1, max_size=5))),
        price_per_meter=draw(st.one_of(st.none(), st.decimals(min_value=1, max_value=500, places=2))),
        price_per_beam=draw(st.one_of(st.none(), st.decimals(min_value=10, max_value=2000, places=2))),
...
class TestEntryDataPersistence:
        ...
        **Feature: entry-page-system, Property 7: Configuration data persistence**
        ...
        attribute_nodes = [
            ...
            for i, field_name in enumerate([
                'type', 'company', 'material', 'opening_system', 'system_series',
                'code', 'length_of_beam', 'renovation', 'width', 'builtin_flyscreen_track',
                'total_width', 'flyscreen_track_height', 'front_height', 'rear_height',
                'glazing_height', 'renovation_height', 'glazing_undercut_height', 'pic',
                'sash_overlap', 'flying_mullion_horizontal_clearance', 'flying_mullion_vertical_clearance',
        ...
        for field_name, field_value in form_data.items():
            if field_value is not None:
                ...
                matching_node = next((node for node in attribute_nodes if node.name == field_name), None)
                if matching_node:
        ...
        for field_name, original_value in original_data.items():
            if original_value is not None:
                ...
                if isinstance(original_value, (int, float)) and isinstance(loaded_value, (int, float)):
                    assert abs(float(original_value) - float(loaded_value)) < 0.01
                else:
                    assert loaded_value == original_value, f"Field {field_name}: {loaded_value} != {original_value}"
    ...
    async def test_property_configuration_selection_mapping(
        ...
        configuration_selections = [obj for obj in added_objects if isinstance(obj, ConfigurationSelection)]
