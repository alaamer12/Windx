...
from app.core.rbac import Role, Permission, ResourceOwnership, Privilege, require
...
class TestAdvancedDecoratorPatternsProperties:
        ...
        for requirement in requirements:
            if isinstance(requirement, Role):
                ...
                if user.role == Role.CUSTOMER.value:
                    ...
                    if requirement.resource not in ["configuration", "quote"] or requirement.action not in ["read", "create"]:
                        return False
                elif user.role not in [Role.SUPERADMIN.value, Role.SALESMAN.value, Role.PARTNER.value, Role.DATA_ENTRY.value]:
                    return False
                ...
                if not any(user.role == role.value for role in requirement.roles) and user.role != Role.SUPERADMIN.value:
                    return False
    ...
    @given(
        ...
        decorator_patterns=st.sampled_from([
            ...
            [
                (Role.CUSTOMER, ResourceOwnership("configuration")),
                (Role.SUPERADMIN,)
            ],
    ...
    async def test_property_multiple_decorator_or_logic_evaluation(
        ...
        **Feature: entry-page-customer-rbac-fix, Property 10: Multiple decorator OR logic evaluation**
        ...
        try:
            ...
            assert at_least_one_satisfied, f"Access granted but no requirement group should be satisfied for user role {user.role}"
            ...
            assert all_groups_failed, f"Access denied but at least one requirement group should be satisfied for user role {user.role}"
    ...
    async def test_property_role_composition_or_logic(
        ...
        for user in users:
            try:
                ...
                assert (
                    any(user.role == role.value for role in role_combinations) or 
        ...
        try:
            ...
            for permission in permission_sets:
                if user.role == Role.SUPERADMIN.value:
                    ...
                    if permission.resource in ["configuration", "quote"] and permission.action in ["read", "create"]:
                        should_have_access = True
                        break
            
            assert should_have_access, f"User {user.role} granted access but shouldn't have permission for any of {[(p.resource, p.action) for p in permission_sets]}"
            ...
            assert should_be_denied, f"User {user.role} denied access but should have permission for at least one of {[(p.resource, p.action) for p in permission_sets]}"
    ...
    @given(
        ...
        complex_requirements=st.lists(
            st.tuples(
                ...
                st.builds(ResourceOwnership,
                         resource_type=st.sampled_from(["configuration", "customer"]))
            ),
            min_size=2,
            max_size=3
        )
    ...
    async def test_property_complex_decorator_combinations_or_logic(
        self, 
        user: User, 
        complex_requirements: list[tuple[Role, Permission, ResourceOwnership]]
    ):
        """
        ...
        try:
            ...
            assert at_least_one_satisfied, f"Access granted but no complete requirement set satisfied for user {user.role}"
            ...
            assert all_failed, f"Access denied but at least one complete requirement set should be satisfied for user {user.role}"

    @given(
        user=user_with_specific_role(),
        privilege_objects=st.lists(
            st.builds(Privilege,
                     roles=st.lists(st.sampled_from([Role.SALESMAN, Role.PARTNER, Role.CUSTOMER]), min_size=1, max_size=2),
                     permission=st.builds(Permission,
                                         resource=st.sampled_from(["configuration", "quote"]),
    ...
    async def test_property_privilege_object_or_logic(
        self, 
        user: User, 
        privilege_objects: list[Privilege]
    ):
        """
        ...
        try:
            ...
            for privilege in privilege_objects:
                if any(user.role == role.value for role in privilege.roles) or user.role == Role.SUPERADMIN.value:
                    satisfies_privilege = True
                    break
            
            assert satisfies_privilege, f"User {user.role} granted access but doesn't satisfy any privilege"
            ...
            assert satisfies_no_privilege, f"User {user.role} denied access but should satisfy at least one privilege"