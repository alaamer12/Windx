...
class TestEnhancedErrorHandling:
    ...
    async def test_save_profile_validation_error_without_field_errors(
        ...
        with patch("app.services.entry.EntryService.save_profile_configuration") as mock_save:
            ...
            assert "detail" in error_data
            detail = error_data["detail"]
            assert detail["message"] == "422: Generic validation error"  # HTTPException includes status code
            assert detail["error_type"] == "validation_error"
    ...
    async def test_save_profile_unexpected_error_handling(
        ...
        with patch("app.services.entry.EntryService.save_profile_configuration") as mock_save:
            ...
            assert "detail" in error_data
            detail = error_data["detail"]
            assert detail["message"] == "An unexpected error occurred while saving the configuration"
    ...
    async def test_error_logging_behavior(
        ...
        with patch("app.services.entry.EntryService.save_profile_configuration") as mock_save:
            ...
            validation_logs = [record for record in caplog.records if "Save Profile Validation Error" in record.message]
            assert len(validation_logs) > 0
            
            # Check that field errors were logged
            field_error_logs = [record for record in caplog.records if "Field Errors" in record.message]
        ...
        with patch("app.services.entry.EntryService.save_profile_configuration") as mock_save:
            ...
            unexpected_logs = [record for record in caplog.records if "Save Profile Unexpected Error" in record.message]
            assert len(unexpected_logs) > 0
            
            # Check that error type was logged
            error_type_logs = [record for record in caplog.records if "Error Type" in record.message]
    ...
    async def test_validation_exception_empty_field_errors(
        ...
        with patch("app.services.entry.EntryService.save_profile_configuration") as mock_save:
            ...
            detail = error_data["detail"]
            assert detail["message"] == "422: Validation failed"  # HTTPException includes status code
            assert detail["error_type"] == "validation_error"
