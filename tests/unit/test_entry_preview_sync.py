...
class TestRealTimePreviewSynchronization:
    ...
    @given(
        ...
        field_updates=st.dictionaries(
            keys=st.sampled_from([
                'company', 'code', 'width', 'front_height', 'rear_height',
                'glazing_height', 'pic', 'weight_per_meter', 'price_per_meter'
            ]),
            values=st.one_of(
    ...
    def test_preview_updates_reflect_form_changes(self, initial_data, field_updates):
        ...
        assert len(initial_preview.headers) == len(updated_preview.headers), "Preview structure should remain consistent"
        ...
        for field_name, new_value in field_updates.items():
            ...
            if header:
                ...
                if field_name not in initial_data or initial_data[field_name] != new_value:
                    assert initial_row[header] != updated_row[header], f"Preview should reflect change in field {field_name}"
                
                # New value should be properly formatted in preview
                formatted_value = self._format_value_for_preview(new_value, field_name)
                assert updated_row[header] == formatted_value, f"Preview should show formatted value for {field_name}"
    ...
    def test_conditional_field_changes_update_preview(self, form_data):
        ...
        if form_data.get('renovation') is not None:
            ...
            assert row[renovation_header] == expected_value, "Renovation field should show boolean value when set"
        else:
        ...
        if form_data.get('builtin_flyscreen_track') is not None:
            ...
            assert row[flyscreen_header] == expected_value, "Flyscreen field should show boolean value when set"
        else:
        ...
        for field_name, field_value in field_sequence:
            ...
            if header:
                ...
                if field_value is not None and field_value != '':
                    ...
                    assert row[header] == formatted_value, f"Field {field_name} should be synchronized in preview"
                else:
                    assert row[header] == 'N/A', f"Empty/null field {field_name} should show as N/A in preview"
    
    @given(
        numeric_updates=st.dictionaries(
            keys=st.sampled_from(['width', 'front_height', 'rear_height', 'weight_per_meter']),
            values=st.floats(min_value=0.1, max_value=5000.0, allow_nan=False, allow_infinity=False),
            min_size=1,
            max_size=4
        )
    )
    def test_numeric_field_updates_preserve_precision(self, numeric_updates):
        ...
        for field_name, field_value in numeric_updates.items():
            ...
            if header:
                ...
                assert row[header] == expected_value, f"Numeric field {field_name} should preserve precision in preview"
        ...
        for field_name, field_value in array_updates.items():
            ...
            if header:
                if len(field_value) > 0:
                    ...
                    assert row[header] == expected_value, f"Array field {field_name} should be comma-separated in preview"
                else:
                    assert row[header] == 'N/A', f"Empty array field {field_name} should show as N/A in preview"
    ...
    def test_preview_synchronization_performance(self):
        ...
        assert generation_time < 0.1, f"Preview generation should be fast (<100ms), took {generation_time:.3f}s"
    ...
    def test_empty_to_filled_form_synchronization(self):
        ...
        assert filled_na_count < na_count, "Filled form should have fewer N/A values than empty form"
