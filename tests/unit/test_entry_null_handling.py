...
def profile_data_with_nulls(draw, manufacturing_type_id):
    ...
    optional_fields = {
        ...
        'reinforcement_steel': draw(st.one_of(st.none(), st.just([]), st.lists(st.text(min_size=1, max_size=50), min_size=1, max_size=3))),
        'colours': draw(st.one_of(st.none(), st.just([]), st.lists(st.text(min_size=1, max_size=20), min_size=1, max_size=5))),
...
def form_data_with_nulls(draw):
    ...
    return {
        ...
        'type': draw(st.one_of(st.none(), st.just(''), st.sampled_from(['Frame', 'Flying mullion']))),
        'company': draw(st.one_of(st.none(), st.just(''), st.text(min_size=1, max_size=100))),
        'material': draw(st.one_of(st.none(), st.just(''), st.sampled_from(['Aluminum', 'Vinyl', 'Wood']))),
        'opening_system': draw(st.one_of(st.none(), st.just(''), st.sampled_from(['Casement', 'Sliding']))),
        'width': draw(st.one_of(st.none(), st.floats(min_value=10.0, max_value=200.0))),
        ...
        'colours': draw(st.one_of(st.none(), st.just([]), st.lists(st.text(min_size=1, max_size=20), max_size=3))),
        'renovation': draw(st.one_of(st.none(), st.booleans())),
    }
    ...
    async def test_property_save_load_null_preservation(
        ...
        attribute_nodes = [
            ...
            for i, field_name in enumerate([
                ...
                'code', 'width', 'renovation', 'colours', 'price_per_meter'
            ], 1)
        ]
    ...
    @given(
        mixed_data=st.dictionaries(
            ...
            st.one_of(
                ...
                st.just([]),
                st.dictionaries(st.text(min_size=1, max_size=10), st.text(min_size=1, max_size=10), max_size=3)
            ),
            min_size=1,
            max_size=15
        )
