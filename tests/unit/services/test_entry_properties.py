...
field_values = st.one_of(
    ...
    st.floats(min_value=-100.0, max_value=100.0, allow_nan=False, allow_infinity=False),  # Reduced range
    st.booleans(),
    st.none(),
...
collection_operators = st.sampled_from([
    "in", "not_in", "any_of", "all_of"
])

# Strategy for generating existence operators
existence_operators = st.sampled_from([
...
def logical_condition_strategy(draw, max_depth=3):
    ...
    if operator == "not":
        return {
            "operator": "not",
            "condition": draw(logical_condition_strategy(max_depth - 1))
        }
    else:
        conditions = draw(st.lists(
            logical_condition_strategy(max_depth - 1),
            min_size=1,
            max_size=3
        ))
        return {
...
condition_strategy = st.one_of(
    simple_condition_strategy(),
    logical_condition_strategy(max_depth=2)
)
...
class TestConditionEvaluationProperties:
    ...
    def test_condition_evaluation_is_deterministic(self, condition, form_data):
        ...
        if (condition.get("operator") == "matches_pattern" and 
            condition.get("value") and 
            any(char in str(condition["value"]) for char in ["[", "]", "(", ")", "*", "+", "?"])):
            assume(False)
    ...
    def test_logical_and_operator_properties(self, condition1, condition2, form_data):
        ...
        for cond in [condition1, condition2]:
            if (cond.get("operator") == "matches_pattern" and 
                cond.get("value") and 
                any(char in str(cond["value"]) for char in ["[", "]", "(", ")", "*", "+", "?"])):
                assume(False)
    ...
    @given(
        numeric_value=st.integers(min_value=-100, max_value=100),
        form_data=form_data_strategy
    )
    @settings(max_examples=30, deadline=1000)
