...
def existing_configuration_data(draw, user_id):
    ...
    return Configuration(
        ...
        total_price=draw(st.decimals(min_value=Decimal("1.00"), max_value=Decimal("15000.00"), places=2)),
        calculated_weight=draw(st.decimals(min_value=Decimal("0.1"), max_value=Decimal("1000.0"), places=2))
    )
...
class TestBackwardCompatibilityProperties:
    ...
    @given(
        ...
        existing_configs=st.lists(
            st.builds(lambda user_id: existing_configuration_data(user_id), 
    ...
    async def test_property_superuser_access_preserved_for_existing_configurations(
        ...
        mock_db, 
        superuser: User, 
        existing_configs: list[Configuration]
    ):
        """
    ...
    @given(
        users_and_configs=st.lists(
            st.tuples(legacy_user_data(), existing_configuration_data(st.integers(min_value=1, max_value=100))),
            min_size=1, max_size=5
        )
    )
    ...
    async def test_property_mixed_legacy_and_new_data_coexistence(
        self, 
        mock_db, 
        users_and_configs: list[tuple[User, Configuration]]
    ):
        """
        ...
        for user, user_config in users_and_configs:
            ...
            with patch('app.core.rbac.RBACQueryFilter.filter_configurations') as mock_filter:
                ...
                user_configs = [config for config in all_configs if config.customer_id == user.id or user.is_superuser]
    ...
    async def test_property_configuration_creation_api_compatibility(
        ...
        entry_service._get_or_create_customer_for_user = AsyncMock(return_value=auto_created_customer)
    ...
    async def test_property_error_handling_backward_compatibility(
        ...
        if error_scenarios == "customer_creation_failure":
            entry_service._get_or_create_customer_for_user = AsyncMock(side_effect=Exception("Customer creation failed"))
            ...
            entry_service._get_or_create_customer_for_user = AsyncMock(return_value=Customer(id=1, email=user.email))
            entry_service.mfg_type_repo.get = AsyncMock(return_value=ManufacturingType(id=1, name="Test"))
