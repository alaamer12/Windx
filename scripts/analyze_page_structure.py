...
async def get_page_structure(
    ...
    manufacturing_type_id: Optional[int] = None,
    ...
    if manufacturing_type_id:
        ...
        stmt_count = (
            ...
            .where(
                ManufacturingType.id == manufacturing_type_id,
...
async def get_page_type_summary(session: AsyncSession) -> Dict:
    ...
    stmt = (
        select(
            AttributeNode.page_type,
            func.count(func.distinct(AttributeNode.manufacturing_type_id)).label('manufacturing_types_count'),
            func.count(AttributeNode.id).label('total_attributes'),
            func.avg(func.count(AttributeNode.id)).over(partition_by=AttributeNode.page_type).label('avg_attributes_per_type')
        )
...
def print_structure(structure: Dict, show_columns: bool = False):
    ...
    if not structure:
        console.print(Panel("❌ No manufacturing types with attributes found.", 
                          style="bold red", box=box.ROUNDED))
        return
...
def print_page_type_summary(summary: Dict):
    ...
    if not summary:
        console.print(Panel("❌ No page types found.", 
                          style="bold red", box=box.ROUNDED))
        return
...
def print_manufacturing_types(manufacturing_types: List[ManufacturingType]):
    ...
    if not manufacturing_types:
        console.print(Panel("❌ No active manufacturing types found.", 
                          style="bold red", box=box.ROUNDED))
        return
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze WindX page structure with beautiful output")
    parser.add_argument(
        "--show-columns", 
        action="store_true", 
        help="Show column names for each page"
    )
    parser.add_argument(
        "--manufacturing-type-id", 
        type=int, 
        help="Analyze specific manufacturing type ID only"
    )
    parser.add_argument(
        "--summary-only", 
        action="store_true", 
        help="Show only page type summary"
    )
