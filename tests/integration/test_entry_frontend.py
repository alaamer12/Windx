...
class TestEntryPageFrontend:
    ...
    async def test_accessories_page_renders(
        ...
        response = await client.get("/api/v1/entry/accessories", headers=superuser_auth_headers)
    ...
    async def test_glazing_page_renders(
        ...
        response = await client.get("/api/v1/entry/glazing", headers=superuser_auth_headers)
    ...
    async def test_navigation_tabs_present(
        ...
        pages = ["/api/v1/entry/profile", "/api/v1/entry/accessories", "/api/v1/entry/glazing"]

        for page_url in pages:
            response = await client.get(page_url, headers=superuser_auth_headers)
    ...
    async def test_profile_page_without_manufacturing_type(
        ...
        response = await client.get("/api/v1/entry/profile", headers=superuser_auth_headers)
        ...
        assert "Profile Data Entry" in html_content
        assert (
            "manufacturing_type_id: null" in html_content
            or "manufacturing_type_id: None" in html_content
        )
    ...
    async def test_profile_page_css_and_js_structure(
        ...
        alpine_directives = ["x-data", "x-init", "x-show", "x-text", "x-model", "@click", "@change"]
