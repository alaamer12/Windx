...
def _create_storage_provider(settings: FileStorageSettings) -> StorageProvider:
    ...
    if provider_name == "local":
        ...
        if not supabase_url:
            raise ValueError(
                "SUPABASE_URL is required when using supabase storage provider"
            )
        ...
        key_str = supabase_key.get_secret_value() if hasattr(supabase_key, 'get_secret_value') else str(supabase_key)
        ...
        raise ValueError(
            ...
            f"Supported providers: local, supabase"
        )
