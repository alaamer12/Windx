...
from app.schemas.entry import ProfileEntryData, ProfileSchema, FormSection, FieldDefinition
...
def field_with_validation_rules(draw):
    ...
    if data_type == "string":
        ...
        if rule_type == "length":
            ...
            validation_rules["pattern"] = draw(st.sampled_from([
            ...
            validation_rules["choices"] = draw(st.lists(st.text(min_size=1, max_size=10), min_size=2, max_size=5))
    ...
    if field.validation_rules:
        ...
        if field.data_type == "string":
            if "min_length" in rules:
                ...
                base_data[field.name] = draw(st.text(min_size=rules["max_length"] + 1, max_size=rules["max_length"] + 10))
            elif "pattern" in rules:
                # Generate string that doesn't match pattern
                base_data[field.name] = draw(st.text(min_size=1, max_size=20).filter(
                    lambda x: not __import__("re").match(rules["pattern"], x)
                ))
            elif "choices" in rules:
                # Generate value not in choices
                invalid_choice = draw(st.text(min_size=1, max_size=20).filter(
                    lambda x: x not in rules["choices"]
                ))
                base_data[field.name] = invalid_choice
            elif rules.get("rule_type") == "email":
                # Generate invalid email
                base_data[field.name] = draw(st.sampled_from([
            ...
            if "min" in rules:
                ...
                base_data[field.name] = draw(st.floats(min_value=rules["max"] + 0.1, max_value=rules["max"] + 100))
...
class TestEntryValidationEnforcement:
        ...
        if field.validation_rules:
            ...
            if hasattr(invalid_data, field.name) and getattr(invalid_data, field.name) is not None:
                if field.name in exc_info.value.field_errors:
                    ...
                    assert field.label.lower() in error_message.lower() or field.name.lower() in error_message.lower()
    ...
    @pytest.mark.asyncio
    @given(
        width=st.floats(min_value=10, max_value=50),
        height=st.floats(min_value=10, max_value=50)
    )
    ...
    async def test_property_valid_data_passes_validation(self, valid_data: dict):
        ...
        **Feature: entry-page-system, Property 6: Schema-based validation enforcement**
        ...
        fields = [
            ...
            FieldDefinition(name="opening_system", label="Opening System", data_type="string", required=True),
            FieldDefinition(name="system_series", label="System Series", data_type="string", required=True),
