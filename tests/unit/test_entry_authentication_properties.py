...
def mock_user_with_role(draw):
    ...
    user.role = draw(st.sampled_from([
        Role.CUSTOMER.value,
        Role.SALESMAN.value,
        Role.PARTNER.value,
...
class TestEntryAuthenticationIntegration:
    ...
    @given(
        user=mock_user_with_role(),
        configuration_id=st.integers(min_value=1, max_value=1000)
    )
    ...
    def test_property_rbac_decorator_consistency(self, users: list, operation: str):
        ...
        **Feature: entry-page-system, Property 9: Authentication integration**
        ...
        assert "user" in param_names, f"Method {operation} should have user parameter for authentication"
    ...
    async def test_property_schema_access_authentication(
        self, user, manufacturing_type_id: int
    ):
        """
        **Feature: entry-page-system, Property 9: Authentication integration**
