...
class TestPermissionChecking(TestRBACService):
    ...
    async def test_check_permission_success(self, rbac_service, user):
        ...
        rbac_service.enforcer.enforce.assert_called_once_with(
            user.email, "configuration", "read"
        )
    ...
    async def test_check_permission_failure(self, rbac_service, user):
        ...
        rbac_service.enforcer.enforce.assert_called_once_with(
            user.email, "configuration", "write"
        )
...
class TestResourceOwnership(TestRBACService):
    ...
    async def test_check_resource_ownership_superadmin(self, rbac_service, superadmin_user):
        ...
        result = await rbac_service.check_resource_ownership(
            superadmin_user, "configuration", 123
        )
    ...
    @pytest.mark.asyncio
    async def test_check_resource_ownership_configuration_not_found(self, rbac_service, user, mock_db):
...
class TestAccessibleCustomers(TestRBACService):
    ...
    @pytest.mark.asyncio
    async def test_get_accessible_customers_superadmin(self, rbac_service, superadmin_user, mock_db):
...
class TestCustomerManagement(TestRBACService):
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_customer_race_condition(self, rbac_service, user, customer, mock_db):
